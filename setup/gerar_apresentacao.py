...
AMARELO       = RGBColor(0xFF, 0xB9, 0x00)   # atenção / semana
...
add_text(s, "Semanas  2 · 3 · 4",
         Inches(0.5), Inches(2.9), W - Inches(1), Inches(0.7),
         font_size=28, bold=True, color=AMARELO, align=PP_ALIGN.CENTER)

add_text(s, "Do prompt genérico ao diagnóstico profissional",
         Inches(0.5), Inches(3.65), W - Inches(1), Inches(0.6),
         font_size=20, italic=True, color=CINZA_CLARO, align=PP_ALIGN.CENTER)

add_text(s, "Trabalho em duplas  ·  1h30 por semana  ·  Evidência gerada ao final",
...
boxes = [
    ("Semana 2", '"O chamado chegou"',     "Debugging com Copilot Chat",   AZUL_ACENTO),
    ("Semana 3", '"O pipeline quebrou"',   "CI com @workspace + MCP",       AZUL_ACENTO),
    ("Semana 4", '"Tudo caiu ao mesmo tempo"', "Agent Mode + Segurança",    AZUL_ACENTO),
]
for i, (sem, tema, desc, cor) in enumerate(boxes):
    ...
    add_text(s, sem,   bx + Inches(0.15), Inches(1.95), Inches(3.7), Inches(0.5),
             font_size=14, color=AMARELO, bold=True)
    add_text(s, tema,  bx + Inches(0.15), Inches(2.4),  Inches(3.7), Inches(0.8),
             font_size=19, bold=True, color=BRANCO)
    add_text(s, desc,  bx + Inches(0.15), Inches(3.15), Inches(3.7), Inches(0.6),
             font_size=15, color=CINZA_CLARO)

add_bullet_box(s, [
    "Trabalho em duplas — um escreve o prompt, o outro revisa antes de enviar",
    "Ao final de cada semana: gerar evidência HTML com os critérios cumpridos",
...
# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 3 — Configuração do ambiente
# ══════════════════════════════════════════════════════════════════════════════
s = prs.slides.add_slide(BLANK)
full_bg(s)
...
requisitos = [
    ...
    ("Node.js LTS",          "nodejs.org  (para a extensão AnythingLLM)"),
]
...
add_text(s, "2. Abra a pasta no VS Code  →  clique duas vezes em  setup.bat  → aguarde (~3 min)",
         Inches(0.55), Inches(6.13), Inches(12.0), Inches(0.45),
         font_size=16, color=BRANCO)

add_text(s, "⚠  Docker Desktop deve estar em execução antes de iniciar o setup.bat",
         Inches(0.3), Inches(7.0), W - Inches(0.6), Inches(0.4),
         font_size=15, bold=True, color=AMARELO, align=PP_ALIGN.CENTER)
...
# ══════════════════════════════════════════════════════════════════════════════
def slide_semana_capa(numero, titulo, papel, hora, ferramenta_cor=AZUL_ACENTO):
    s = prs.slides.add_slide(BLANK)
    full_bg(s, AZUL_ESCURO)
    add_rect(s, 0, 0, W, H, AZUL_ESCURO)
    add_rect(s, 0, Inches(2.2), W, Inches(3.1), AZUL_MEDIO)
    add_rect(s, 0, Inches(2.2), Inches(0.15), Inches(3.1), ferramenta_cor)

    add_text(s, f"SEMANA {numero}",
             Inches(0.5), Inches(0.5), W - Inches(1), Inches(0.65),
             font_size=20, bold=True, color=ferramenta_cor, align=PP_ALIGN.CENTER)
    add_text(s, f'"{titulo}"',
             Inches(0.5), Inches(1.1), W - Inches(1), Inches(1.2),
...
def slide_cenario(semana_n, ticket, situacao, itens, cor=AZUL_ACENTO):
    s = prs.slides.add_slide(BLANK)
    full_bg(s)
    slide_header(s, f"Semana {semana_n} — Cenário", ticket, accent=cor)
    add_text(s, situacao,
...
def slide_objetivo(semana_n, objetivos, ferramenta, prompt_arquivo, cor=AZUL_ACENTO):
    s = prs.slides.add_slide(BLANK)
    full_bg(s)
    slide_header(s, f"Semana {semana_n} — Seu objetivo", accent=cor)
...
def slide_criterios(semana_n, criterios, cor=AZUL_ACENTO):
    s = prs.slides.add_slide(BLANK)
    full_bg(s)
    slide_header(s, f"Semana {semana_n} — Critérios de aprovação", accent=cor)
...
# SEMANA 2
# ══════════════════════════════════════════════════════════════════════════════
slide_semana_capa(2, "O chamado chegou", "Analista N2", "~1h30", AZUL_ACENTO)

slide_cenario(2, "TICKET-001 — Falha no Processamento de Pedidos (P1)",
    "São 14h15 de segunda. A Farmácia Boa Saúde — cliente VIP — está ligando há 15 min.\nO N1 (AnythingLLM) já fez a triagem. SLA: 2 horas.",
    [
        "Pedidos da Farmácia Boa Saúde estão falhando na API",
        "O N1 indica: TypeError em validar_pedido() quando quantidade=None",
...
slide_objetivo(2,
    [
        "Abrir o ticket-001.md e ler o diagnóstico do N1",
...
# SEMANA 3
# ══════════════════════════════════════════════════════════════════════════════
COR_S3 = RGBColor(0x00, 0x99, 0x88)  # teal
slide_semana_capa(3, "O pipeline quebrou", "Dev de Plataforma", "~1h30", COR_S3)

slide_cenario(3, "TICKET-002 — Pipeline de Produção Bloqueado (P2)",
...
# SEMANA 4
# ══════════════════════════════════════════════════════════════════════════════
COR_S4 = RGBColor(0xCC, 0x44, 0x00)  # laranja-vermelho urgência
slide_semana_capa(4, "Tudo caiu ao mesmo tempo", "Dev + Suporte", "~1h30", COR_S4)

slide_cenario(4, "TICKET-003 — Incidente Crítico: Múltiplas Falhas Simultâneas (P1)",
...
s = prs.slides.add_slide(BLANK)
full_bg(s)
slide_header(s, "Gerar a evidência ao final de cada semana")
...
add_bullet_box(s, [
    "Substitua o número da semana: --semana 2, --semana 3 ou --semana 4",
